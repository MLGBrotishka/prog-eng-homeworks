...
class ServiceClient:
    ...
    async def patch(self, path, json=None, **kwargs):
        """Make PATCH request"""
        url = f"{self.base_url}{path}"
        print(f"[DEBUG] PATCH {url}", file=sys.stderr)
        try:
            async with self.session.patch(url, json=json, **kwargs) as resp:
                text = await resp.text()
                print(f"[DEBUG] Response status: {resp.status}, text_len: {len(text)}", file=sys.stderr)
                return Response(resp.status, text, resp.headers)
        except Exception as e:
            print(f"[ERROR] PATCH request failed: {e}", file=sys.stderr)
            raise
